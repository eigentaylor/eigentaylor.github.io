...
thumbnail: /assets/img/lichtman.jpg
importance: 5
tags: 
category: polisci
glossary: polisci
...
Update (March 2026): I have added more detail about [the 2016 switch](#the-2016-switch), which gives more credit to Lichtman but is more damning to his model, along with other minor edits and clarifications.
...
The <d-glossary key="electoral_college">Electoral College</d-glossary> is a whole thing, which complicates the relationship between the popular vote and the outcome of the presidential election. Suffice it to say, the candidate who gets the most votes across the entire country does not necessarily win the presidency, and a candidate must win enough states to get a majority of the electoral votes.
...
- 1876: Samuel Tilden won the popular vote by approximately 3 percentage points, but Rutherford B. Hayes won the presidency after a controversial electoral commission awarded him 20 disputed electoral votes. Fun fact: if the <d-glossary key="apportionment">apportionment</d-glossary> in 1870 had been done correctly, Tilden would have won the presidency with a clear electoral vote majority.
...
In essence, the <d-glossary key="electoral_college">electoral college</d-glossary> is incredibly chaotic. This is, in my opinion, why I think Lichtman was absolutely right to focus on predicting the popular vote, rather than the electoral college. Trying to predict individual states is simply too difficult.
...
- There have been major foreign policy or military failures.
...
In 1968, Nixon won the popular vote by just 0.7%, but the country was deeply divided over the Vietnam War and civil rights issues. The presence of George Wallace as a third-party candidate further complicated the electoral landscape, but data has shown that [Wallace voters would have likely broke mostly for Nixon](https://www.pbs.org/wgbh/americanexperience/features/wallace-1968-campaign). Thus, I do believe that the real winner of the popular vote was indeed Nixon. I strongly believe Nixon was the <d-glossary key="condorcet_winner">Condorcet winner</d-glossary> in 1968.
...
Therefore, if we can identify a set of governance failures which have historically correlated with the incumbent party losing the popular vote, then we can use these as predictors for the outcome of the popular vote, so long as we can be reasonably sure that the electorate's perception of governance is relatively well-aligned with this identified set.

This does, however, require a functional information ecosystem, where the electorate is not being misled on a massive scale. As media fragmentation and misinformation increase, this alignment is likely to weaken, reducing the predictive power of such a model.
...
But, all things considered, six false keys does seem to be a relatively reasonable threshold for predicting the popular vote. No incumbent party has ever won the popular vote with more than six false keys, though the Democrats did in 2016. Further, no incumbent party has lost the popular vote with 4 or fewer false keys, though the Democrats did in 2024. My primary objection is taking that and then claiming that it actually predicts the popular vote, rather than just being correlated with it. I did calculate a correlation of $r = -0.828$ between the number of false keys and the Democratic margin in the NPV from 1984-2024. This is a strong negative correlation, but it is not perfect, and it does not justify a hard threshold of 6 keys.
...
In particular, if we are to believe that the keys act as a foolproof function from

$$2^{13}\to \{\text{Incumbent Party Wins NPV}, \text{Challenger Party Wins NPV}\}$$

then we need to be able to determine the truth value of each key in a consistent and objective manner. Otherwise, the model is not really well-defined.
...
Let us set aside the fact that these reasons fell apart in 2024, and take a moment to look at Lichtman's actual published predictions.

There is one important thing to Lichtman's credit. Comparing his published predictions from 2012 to 2016 gives something striking. In 2012, he *specifically* says Democrats are predicted to win the **popular vote**. In 2016, the mention of the popular vote is no longer present in the final table, and merely says "Keys Favor the Republican Candidate in 2016", while keeping a line about the popular vote earlier in the prediction. This an interesting detail because it is evidence that Lichtman might have been truly foreseeing a divergence between his prediction and the popular vote.

Lichtman might have had true foresight that Trump would win the election, but might have had some serious doubts about whether he would win the popular vote. And given the state of the polls showing a high likelihood that Clinton would sweep the popular vote, this is understandable. He might have felt boxed in by the popular vote distinction, but also genuinely believed Trump would defy the odds. He still includes the same quote in the 2016 prediction that "the Keys predict the popular vote, not the state-by-state tally of Electoral College votes." (The Post Rider), but there does appear to be some hedging.

But there's something strange about this claim about the inevitability of Democrats winning the popular vote. When Lichtman supposedly made this deduction, Bush had won the popular vote just 12 years and only two presidential elections prior (prior to 2016 election night). Since that election, Obama won the popular vote fair and square in 2008 and 2012. It does not seem plausible that Lichtman could foresee that the popular vote would diverge from the electoral college in 2016, particularly since he made no mention of this. The idea that Democrats would be heavily favored to win the popular vote, and Republicans would only have a path in the electoral college is something that does not seem plausible to conclude until after the 2016 election results were in. It is not something that would have been obvious to conclude before the election, given the recent history of the popular vote and the state of the polls.

And, of course, the fact that this prediction was completely wrong in 2024 only further undermines the credibility of this claim, unless you say that 2024 was not a "close election", which seems dubious to me. Though I talk about this more in the conclusion. Suffice it to say, I do not believe Lichtman's reasons for switching what the keys predict are at all credible. Particularly due to the timeline issues.

The reasons for the switch are either an incredible deduction made just before election night, for which there is no evidence, or it's a completely post-hoc rationalization that turned out to be completely wrong in 2024. Is it not far more likely that he saw the election results, saw that Trump won the electoral college but lost the popular vote, and then retroactively decided to change his model to fit the results? You may have heard the term "moving the goalpost" before, and that is exactly what this would be.

And, I have to take a second to mention the most ridiculous justification that Lichtman gives: that he said Trump would be impeached and that would make no sense if he wasn't predicting who would govern. To be clear, the argument he is rebutting is that "if the keys predict the popular vote, and Republicans lost the popular vote in 2016, then the keys were wrong". Lichtman counters this by adding a hidden supposition that we are claiming that Lichtman expected Trump to win the popular vote, **but not the electoral college**. This is then clearly wrong because Lichtman made statements about impeachment which only make sense if Lichtman predicted Trump would govern. I have never heard anyone claim that Lichtman predicted Trump would win the popular vote but not the electoral college, and it is a complete strawman. It seems most plausible Lichtman predicted Trump would win at least the electoral college, but also likely the popular vote since a divergence between the two is extremely rare.

It's quite funny to watch Lichtman in interviews try to explain this. [In one interview with Kamal Ahmed](https://youtu.be/zSJjxt3Mmok?si=a3y32caXEeez_-Tx&t=195), Lichtman shouted at Ahmed to "**STOP**", claiming that he was spreading "misinformation", and threatening to end the interview right then and there.

However, whether or not the switch from popular vote to electoral college was before or after the election, the fact there is a switch *at all*, more than anything, breaks the credibility of Lichtman's keys. He changed what his model measured **without changing any aspect of his methodology**. This is completely unscientific. Because every reason he gave for why he could *only* predict the popular vote, which is *still* in his book, applies equally before and after 2016. And the reasons he gave for the switch didn't even last two elections, as the popular vote flipped back in 2024.

I would like to posit an alternate theory for what happened in 2016. I think the hedging in his 2016 prediction indicates that Lichtman really did have true insight that Trump would potentially at least win the electoral college. I think he was picking up on the fact that there would be a rejection of the incumbent party, potentially localized in such a way that it would lead to a divergence between the popular vote and the electoral college. Therefore, he hedged his prediction by being unclear in what exactly he was predicting apart from the fact that the Republicans were "favored".

If Trump won the popular vote, the electoral college, or both, then the ambiguous "Keys Favor the Republican Candidate in 2016" prediction would have been "correct" because it's simply unclear what he's even predicting, and is therefore only falsifiable if Clinton won both the popular vote and the electoral college. But the loss of the popular vote and win of the electoral college would require some serious backtracking on many of his claims about what the keys predict over the years. Still, if Lichtman genuinely predicted Trump would win the electoral college in 2016, that would still make the 2016 call the landmark prediction he claims it is, but shatters the credibility of the keys, because, as defined by Lichtman, they predict the popular vote, not the electoral college.

This draws a clear line between Allan Lichtman, the historian and expert political analyst, and his prediction model. I am genuinely willing to cede that Lichtman predicted the "true" winner of every election from 1984 to 2020 (if you are loose about what a "true" winner means), and that he has real insight into the factors that influence presidential elections. However, the model he uses to make these predictions is not at all scientifically rigorous, and I believe it was wrong in 2016 before it was wrong in 2024.

Now, The Post Rider article goes into much more depth about this general topic of the switch, and I highly recommend reading it if you want to understand the full extent of the issues with Lichtman's explanation of 2016. But, it ultimately doesn't, in my opinion, change the fact that there are aspects of Lichtman's model that I think are quite reasonable, but that his particular implementation of those ideas is *deeply* flawed. Regardless of how impressive his personal prediction of 2016 was, the fact remains that his explanation for it in terms of his model is not at all credible, and I have no confidence that his model can actually predict the next President or the winner of the popular vote in a rigorous and scientifically valid way, apart from being a rough predictor with decent correlation.
...
In conclusion, I would like to once again highlight the distinction between Lichtman the historian and political analyst, and his keys model. Lichtman's focus on the popular vote and his ideas about the role of governance in elections over campaigning are genuinely novel and underappreciated. The elections where his model struggled the most, 2016 and 2024, are the ones which arguably most vindicate the axioms that underlie his model--but not the model itself. I do believe his call of Trump in 2016 is a landmark prediction, even if his model failed by every metric Lichtman himself had previously set.

[hyperlink](https://youtu.be/bl8Z7Dl7P9A?si=bEbi9eCpQMFgyxeb)
