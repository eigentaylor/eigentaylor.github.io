...
Okay, so what is approval voting (or AV)? If it satisfies IIA and Pareto efficiency, it must be super complicated, right? Actually, it's dead simple. In fact, it's arguably even simpler than plurality. 

> In approval voting, each voter can vote for as many candidates as they want. The candidate with the most votes wins.

I would argue it's the simplest non-dictatorial system that exists. Plurality is basically approval voting with the added requirement of one vote per voter. This simplicity is part of what makes AV so robust.
...
Now, let's see why it satisfies IIA. The workaround is that we've left rankings behind. Instead of a strict ranking, for which adding a new candidate requires comparing that new option to all existing options, we instead essentially ask voters to group candidates into two categories: those they approve of, and those they do not. In other words, the candidates for which the outcome is satisfactory, and those for which it is not. Note, this means that instead of $$n!$$ possible ballots, or $$\operatorname{floor}(e\,n!)$$ if one need not rank all candidates (fun proof to show this is the formula), we only have $$2^n$$ possible ballots (each candidate is either approved or not approved). This is a massive reduction in complexity, particularly once you have 4 or more candidates.
...
In the above scenario, $$B$$ loses due to a mere 0.1% difference in first choice support between $$B$$ and $$C$$. The idea that $$B$$ is so "bland" and that's why they would have lost the first round appears suspect.

Suppose we instead used approval voting. Assume all voters approve of their first and never their last choice, and the difference is in whether or not they approve of their second choice. In this scenario, it's extremely likely that $$B$$ will win from all the potential second place approvals.
