...
toc:
  - name: Introduction
    subsections: 
      - name: Plurality Problems
      - name: Ranked Voting Systems
      - name: What Makes a Good Voting System?
...
A lesser known example of plurality disenfranchising voters is the 1970 New York Senate election, where James Buckley (the conservative candidate) won with only 39% of the vote, compared to 37% and 24% for two liberal candidates, Richard Ottinger and Charles Goodell, respectively. Together, the liberal candidates had 61% of the vote, showing that a majority of voters likely preferred a more liberal candidate over the conservative Buckley. However, due to the spoiler effect, the election wasn't decided based on what the voters at large wanted, but rather which candidate had the largest and most fervent base of first-choice support.

Speaking of New York, the recent Mayoral race this year had a major focus of getting candidates with no chance to drop out of the race, to avoid splitting the vote. Curtis Sliwa and Eric Adams were both essentially bribed to drop out so that Andrew Cuomo could have a better chance of defeating Zohran Mamdani. 
...
Proponents of ranked systems like RCV often tout the feature of ranking candidates as a way to express more nuanced preferences. And, indeed, This is an appealing step up from plurality's "pick only one" mentality! However, the issue comes with the fact that ranks are actually a lot less granular than one might think (not to mention, easily manipulated strategically).
...
I am going to steal an example mentioned in a comment featured in this [video from PBS Infinite Series](https://youtu.be/AhVR7gFMKNg?si=-n0SSAQ28doyeqoJ&t=852){target="_blank"}: Consider the option between
- $11
- $10
- $1
- A punch to the face

I think most of us would rank the options in the order given above. But how much better is $10 compared to $1? Am I okay with only getting $1? How much better is $11 compared to $10? How much more do I really value $11 over $1 versus $10 over $1? It's also very difficult to really express that I would strongly prefer $1 over a punch to the face.
...
Okay, it's time for math. Okay, not that much, but a little. Without getting into the weeds of the proof of Arrow's theorem ([Veritasium has a very accessible video on it](https://www.youtube.com/watch?v=qf7ws2DF-zk)), I want to give you a very rough idea of what exactly it *says* is impossible. This is a math blog after all!

For an election, we essentially have two desirable ideals for a voting system:
1. Responsiveness: The result of the election should reflect the preferences of the electorate.
2. Stability: The result of the election should not be unduly influenced by irrelevant factors. 

Intuitively, one can think of one important aspect of stability being that the introduction of a new candidate should not change the relative ranking of existing candidates (i.e. no spoiler effect). If Gore beats Bush in a two candidate race where nobody is allowed to vote for anyone else, then introducing Nader should not cause Bush to suddenly jump over Gore in the final ranking if nobody swapped the relative rankings of Gore and Bush (ex. no Gore voters moved Bush above Gore, and no Bush voters moved Gore above Bush).

We articulate these two ideals formally and mathematically as follows:

1. **Pareto Efficiency**: If every voter prefers candidate $$A$$ over candidate $$B$$, then the final result should have $$A$$ ranked higher than $$B$$. This is a formalization of a relatively minimal responsiveness property. 
2. **Independence of Irrelevant Alternatives (IIA)**: The idea is that if everyone in the electorate does not change their relative rankings of two candidates $$A$$ and $$B$$, then the final result should not change the relative ranking of $$A$$ and $$B$$. This is a formalization of the idea of "stability" mentioned earlier.

To get a sense of what IIA means, let's continue our example of Gore and Bush. If Gore beats Bush in a head-to-head matchup, and then we introduce Nader, then we would ideally prefer that either

1. The winner of the election is still Gore, or
2. The new winner is Nader.

This is slightly different from the actual definition of IIA (which is actually about more than just the winner), but it captures the spirit of it. A more accurate articulation would be that we wish that as long as nobody changes their relative positioning of Gore and Bush, then no matter how people change any other aspects of their vote, then the final result should at *least* still have Gore ranked higher than Bush. 
...
Clearly, IIA was violated in the actual 2000 US Presidential Election. Gore would almost certainly have beaten Bush in a head-to-head matchup, but the introduction of Nader arguably caused Bush to win instead.

In essence, you can think of IIA as saying there can be no "spoiler effect" in elections. Like... not actually but just vibe it.
...
One can show that a system like Plurality Voting (FPTP) satisfies Pareto efficiency, but not IIA. The spoiler effect example above is a classic demonstration of the failure of IIA as we have discussed, but we can also see a clear basic responsiveness. If every voter prefered Gore over Bush, then Bush would not have gotten a single vote, and thus Gore have had to get more votes than Bush. Thus, Pareto efficiency is satisfied. 
...
Now, it gets even worse for ranked voting systems. The Gibbard-Satterthwaite theorem states that every non-dictatorial ranked voting system with at least three candidates is susceptible to strategic voting. 
...
The proof of the Gibbard-Satterthwaite theorem relies on showing that in a ranked system satisfying strategyproofness (i.e. no voter can get a strictly better outcome by being dishonest), any blocking set contains a smaller blocking set. Repeatedly applying this idea, we can whittle down any blocking set to a single voter, who is then a dictator. What this shows is that strategyproofness is (like IIA) a property so strict that it can literally only be satisfied by dictatorship.
