...
Addendum 2/6/2026: I do not hold all the views expressed in this post anymore, and I might suggest reading [this post instead](../practicalapproval){:target="_blank"}. I was still in the early stages of my exploration of voting systems when I wrote this post, so there are some major innacuracies and statements I no longer stand by.

In particular, I have been doing a lot more reading and research on voting systems, and I have come to the conclusion that RCV is actually much worse than I initially thought. I will generally preserve the text of the post as it was made in my early exploration of electoral reform and voting systems. However, I plan to write more posts in the future that discuss the issues with RCV in more depth.

I will strike out things that I can no longer justify, and include some "Addendum" notes. At the moment, I cannot possibly justify or advocate for support of RCV. I believe it is a hazard and danger to electoral reform as a whole. The only reform I can now support is Approval voting. For all the flaws that exist in plurality, there are other considerations to make about simplicity, transparency, and trust that cannot be taken for granted. I currently do not fully trust systems like STAR or Ranked Robin, which I think are the best non-Approval alternatives currently in the conversation, over our current plurality system. However, both are far more preferable than RCV.

In short: If we do not replace plurality with approval voting, we may risk the electoral reform movement as a whole. At this moment, I think plurality is actually safer than any non-Approval alternative, *particularly* RCV. If we want this done, it *must* be done right.

You can read more in these newer posts or check [here](https://eigentaylor.github.io/blog/tag/voting/){:target="_blank"} for all my voting-related posts:

- [A Practical Case for Approval Voting](../practicalapproval){:target="_blank"}
- [Approval voting is the Only Internally Consistent Cardinal Method](../consistentcardinal){:target="_blank"}
- [It's Time to Ditch RCV](../ditch-rcv){:target="_blank"} (Coming Soon)

However, if you want a glimpse into my early thoughts on voting systems, feel free to read on. Keep in mind there are innacuracies and take it with a grain of salt.

## Introduction

Updated 12/5/2025, and added addendums on 2/6/2026.

![I'm back — Misha (Pokémon GO)](/assets/img/im-back-misha.gif)
...
Addendum (2/6/26): I no longer support RCV. See [the added note](#a-brief-update){:target="_blank"} above for more details.
...
Addendum (2/6/2026): One other issue with ranked systems is that they often do not use the data "properly". When a losing candidate was ranked over the winner by a majority of voters, that is a clear violation of the so-important ranked data we had to collect. RCV is such a system that does not use its data properly, leading to trust-eroding results that lead to repeal efforts.
...
Rather, RCV instead looks to eliminate candidates until one candidate has majority support over all remaining candidates (in terms of the remaining ballots, at least). But, we will go into quite in-depth about how problematic this can be later.
...
2. The system is robust and utilizes the data provided by voters in a consistent and intuitive way.
...
Addendum (2/6/2026): I now value the Condorcet criterion a lot less as a sort of golden rule. Later in the post, I do explain why it's not necessarily the best, so my position has not changed that much. However I should clarify my updated position on it: **when a system does not elect a Condorcet winner (based on the ballot data), then you create a legitimacy crisis that erodes trust in the system.** That is, if there is a candidate who is apparently preferred by a majority over every other candidate, based on the ballot data, then that candidate must win if we want voters to trust the system and not feel like the system swindled them out of the majority choice, or that majority rule was violated. If there is no data in the ballots to suggest the winner wasn't the Condorcet winner, then there is no problem, in my view. See [this post](../consistentcardinal){:target="_blank"} which proves that Approval (and plurality voting) are one of the few systems to satisfy this, or [this post](../practicalapproval){:target="_blank"} which explains the importance of this.
...
A more recent (though also more debated) [example happened in a 2022 special election in Alaska](https://arxiv.org/pdf/2209.04764v3) (see this paper by Adam Graham-Squire and David McCune for more details).
...
5. Approval Voting is actually also (trivially) pareto efficient! If every voter ranks $$A>B$$, then that literally means that every voter approves of $$A$$ and none approves of $$B$$, so $$A$$ must strictly beat $$B$$ (in fact, 100% to 0%). In effect, this is super Pareto efficiency.
...
2. RCV ~~is strictly better than plurality, but~~ is flawed. It's not monotonic, can eliminate Condorcet winners, and insincere voting can be optimal.
...
4. Gibbard-Satterthwaite shows that no minimally responsive ranked system can be strategyproof without being dictatorial. There can always be a case where dishonesty is optimal.
...
Addendum (2/6/26): This last sentence is important. RCV's pathologies have already occurred in practice, and they have led to major efforts to repeal RCV. It is not a safe long-term solution. See [the added note](#a-brief-update){:target="_blank"} for more details.
...
Addendum (2/6/26): I no longer agree plurality is actually terrible enough to be worse than RCV. See [the added note](#a-brief-update){:target="_blank"} for more details.
...
Forgive me for getting up on my soapbox, but the United States is nearly as polarized as it was before the Civil War. This month, [one poll](https://www.washingtonpost.com/politics/2025/11/02/trump-democrats-poll-post-abc-ipsos/) showed that while over 85% of Republicans approve of Donald Trump, 95% of Democrats disapprove of him. Independents are also 30% approve and 69% disapprove. This is an astounding level of polarization.
...
Addendum (2/6/26): RCV would not likely fix polarization. It could exacerbate it, in fact. See [this post](../practicalapproval/){:target="_blank"} for more details. In that post you will find mention of extensive literature that indicates Approval would reduce polarization.
...
If you find yourself inspired to advocate for approval voting, I recommend checking out [The Center for Election Science](https://electionscience.org/), which advocates for approval voting and has resources to help you get started.
