...
I think most of us would rank the options in the order given above. But how much better is 10 dollars compared to one dollar? Am I okay with only getting a dollar? How much better is 11 dollars compared to 10? How much more do I really value 11 dollars over 1 versus 10 dollars over 1? It's also very difficult to really express that I would strongly prefer a dollar over a punch to the face.
...
4. AV is actually also (trivially) pareto efficient! If every voter ranks $$A>B$$, then that literally means that every voter approves of $$A$$ and none approves of $$B$$, so $$A$$ must strictly beat $$B$$ (in fact, 100% to 0%). In effect, this is super Pareto efficiency.

Now, to be fair, these are some pretty big assumptions. I would not be so bold as to claim that AV is *actually* strictly Pareto efficient in practice (it is definitely weakly Pareto efficient, though). In reality, voters generally have multiple tiers of preference. For example, we could have
...
1. Plurality is **really** bad
2. RCV is strictly better than plurality, but is still flawed. It's not monotonic, can eliminate Condorcet winners, and unsincere voting can be optimal.
3. Arrow's theorem shows that no ranked system can satisfy desirable properties like IIA and Pareto efficiency simultaneously, without being dictatorial.
4. Gibbard-Satterthwaite shows that no minimally responsive ranked system can be strategyproof without being dictatorial.
5. We sidestepped these impossibility theorems by using approval voting, which is not a ranked system.
6. Approval voting is monotonic, satisfies IIA, and is weakly Pareto efficient. Further, under dichotomous preferences, it is sincere, strongly Pareto efficient, and strategyproof. It also almost entirely eliminates the spoiler effect.
7. Approval voting guarantees that if there are Condorcet winners (with respect to the approve/disapprove partition), they will win the election. In fact, $$A$$ will get more approval votes than $$B$$ if and only if $$A$$ beats $$B$$ in a head-to-head matchups of strict approvals (and will do so by the exact same margin).
8. Approval votes are highly stable with respect to overall electorate satisfaction, even in very close elections. If two candidates have similar approval ratings, then the difference in overall satisfaction is bounded by the difference in approval votes.
