...
Note that this also gives a trivial proof that an ACW always exists, since approval voting always produces at least one winner. Further, it shows that there are never any cycles in the strict approval head-to-head matchups, since if $$A$$ beats $$B$$ in strict approvals, and $$B$$ beats $$C$$ in strict approvals, then $$A$$ must have more total approvals than $$B$$, and $$B$$ must have more total approvals than $$C$$, so $$A$$ must have more total approvals than $$C$$, meaning that $$A$$ beats $$C$$ in strict approvals. Thus, the strict approval head-to-head matchups are always acyclic.
