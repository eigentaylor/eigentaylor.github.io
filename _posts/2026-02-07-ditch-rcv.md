...
Even other methods like STAR and Ranked Robin satisfy these criteria. Ranked Robin in particular would potentially be very fast to count on election night across multiple precincts, if a Condorcet winner exists. STAR might be somewhat slower due to the runoff, since precincts need to count both the scores and the pairwise comparisons, but it's still *significantly* faster than RCV, and all of these methods are categorically faster and easier to audit than RCV.
