---
layout: distill
title: Strategic Resiliance of the Better Choices System
date: 2026-07-02
...
The [Better Choices system](../better-choices/){:target="_blank"} is a very interesting flavor of Condorcet where voters vote in all three head-to-head matchups between the candidates. The winner is the candidate who wins the most head-to-head matchups. If there is a tie, the winner is determined by minimax (the candidate with the least bad loss).

For example, if Alice wins both of her head-to-head matchups against Bob and Clark, then Alice is the winner outright. Suppose instead that Alice beats Bob by 100 votes, Bob beats Clark by 200 votes, and Clark beats Alice by 300 votes. Then Bob loses by the least amount (100 votes) and is the winner.
...
which we denote suggestively as $(X_1>Y_1)<(X_2>Y_2)<(X_3>Y_3)$, where $X_i$ and $Y_i$ are distinct candidates. The interpretation is that $X_1$ defeats $Y_1$ by the smallest margin, $X_2$ defeats $Y_2$ by the middle margin, and $X_3$ defeats $Y_3$ by the largest margin. However, formally, we would say that $P(1)=(X_1,Y_1)$ and $P(1)(1)=X_1$ and $P(1)(2)=Y_1$, and similarly for $P(2)$ and $P(3)$. We force that $P(0)=(Y_1,X_1)$, the reverse of the smallest margin matchup.

Note that there are $3!\times 2^3=48$ possible states for 3 candidates, since there are $3!$ ways to order the matchups and $2^3$ ways to choose the winner of each matchup. In general, for $n$ candidates, there are $n!\times 2^{\binom{n}{2}}$ possible states.

The only way for the result of an election to possibly change is to change the relative sizes of the matchups. For example, if a coalition were to attempt to manipulate the election by swapping their sincere votes for $Y_2>X_2$ with $X_2>Y_2$, then a sufficiently coordinated and large coalition might end up making the $X_2>Y_2$ matchup the largest margin, and the previous largest margin matchup $X_3>Y_3$ would then become the middle margin matchup. Or, perhaps, the coalition might be able to change sincere votes for $X_1>Y_1$ into $Y_1>X_1$, which would reverse the smallest margin matchup such that $P(1)=(Y_1,X_1)$, meaning that $Y_1$ now *defeats* $X_1$ by the smallest margin.
...
We denote $$\sigma\in\{(0,1),(1,2),(2,3)\}\subset S_4$$ as permutations on the matchups themselves, and we define

$$\sigma P(i)=P(\sigma(i))$$

For example, if $P=(A>B)<(B>C)<(C>A)$, then $(1,2)P=(B>C)<(A>B)<(C>A)$, and $(0,1)P=(B>A)<(B>C)<(C>A)$.

That is, $(1,2)$ and $(2,3)$ swap the relative strengths of the matchups, while $(0,1)$ reverses the smallest matchup.
...
We consider three cases exhaustively. Let $X_1>Y_1$ be the smallest matchup, and $Q=(0,1)P$ be the result of the swap. Then we have two cases:
...
If $Q$ is still in a cycle, then $f(P)=P(1)(2)=Y_1$ and $f(Q)=Q(1)(2)=X_1$. However, the swap of the second kind is only possible if the coalition sincerely preferred $X_1\succ Y_1$. Hence, $f(P)\succ f(Q)$, and the manipulation is not profitable.
...
Therefore, we need *only* consider the $(1,2)$ swap of the first kind, which swaps the relative strengths of the two weakest matchups, on $P$ which contains a cycle.
...
All of these theorems easily generalize to more than three candidates, except for the last theorem. The first three theorems are independent of the number of candidates. What we find is that the *only* way to potentially profit from a simple manipulation is to start in a cycle, and to swap the relative strengths of the two weakest matchups, which are structured precisely in such a way that
...
In practice, to subvert or profitably manipulate this election, you would have to
...
I do not see this as a practical concern. If you are a voter voting in such an election, I cannot recommend enough that you vote sincerely. There is effectively no way to do this without an unrealistic amount of knowledge, foresight, confidence, and *resources*.
