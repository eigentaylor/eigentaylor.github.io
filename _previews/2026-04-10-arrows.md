...
You may read an intuitive explanation of the above proof, using our friend Victor and a group called the Bob Fanclub, by expanding the intuition block below!
...
- Similarly, everyone but the Bob Fanclub agrees that Alice is better than Bob, so society must rank Alice higher than Bob by our assumption that any strict subset of size $n$ can be overruled. (society ranks Alice $>$ Bob)
...
- the turnout of the election is strictly bigger than the size of the Bob Fanclub plus one (Victor) ($\vert V\vert > n+1$)
- and that a set of voters of size $n+1$ (ex. Victor and the Bob Fanclub) can be overruled.
...
[Maniquet and Mongin (2015)](http://www.jstor.org/stable/43662604) showed that Approval voting is actually entirely compatible with all of Arrow's conditions (and more) on the dichotomous domain. That is, as a function which takes in partitions of the candidates into two tiers--those each voter approves of and those they don't--then Approval voting is essentially Arrovian and a full exception to Arrow's theorem:

> **Theorem:** (Maniquet and Mongin, 2015) Approval voting, as a function from dichotomous preferences to societal rankings, among such functions uniquely satisfies unanimity, IIA, anonymity (and hence non-dictatorship), and Pareto indifference.

Anonymity, which is a stronger version of non-dictatorship, means that the voting system treats all voters equally. Pareto indifference means that if every voter is indifferent between $a$ and $b$, then society should also be indifferent between $a$ and $b$. Approval voting satisfies a bunch of other properties, when viewed this way, but this is sufficient to say it is a technical exception to Arrow's theorem on the dichotomous domain.

Also as mentioned in my post on [IIA](../iia){:target="_blank"}, if you extend IIA to rated voting systems via assuming that voters score candidates independently, then basically all rated voting systems trivially satisfy IIA and thus break Arrow's theorem. If you maintain the requirement via *relative* ordinal rankings, then [only Approval voting](../consistentcardinal){:target="_blank"} satisfies IIA among rated systems, since the scores are directly tied to the ordinal preferences expressed via the ballots. In that post, I also mention a few other proofs that Approval voting satisfies IIA, such as the fact that Approval voting is [strategyproof](https://doi.org/10.2307/1955105) on the dichotomous domain, and that [strategyproofness (loosely) implies IIA (Vorsatz, 2007)](http://www.jstor.org/stable/41106808).
