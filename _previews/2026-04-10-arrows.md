...
Fishburn's proof actually goes a different route, and shows that if you have a "responsive", "stable" (IIA) voting system that *isn't* a dictatorship, then you must have *infinitely many voters*. Which is too mathematically cute for me *not* to write a blog post about!
...
Thus, we get that Victor can be overruled on Alice over Bob and Bob over Alice. This can be applied to any pair of candidates, so Victor has been showed to be powerless and inconsequential in the decision of any pair of candidates, should every other voter disagree with him on that pair.
{% endproof %}
...
As an exercise, use Proposition \ref{single-block} to show that it's impossible to establish a societal ranking for this profile if we assume the system is Arrovian (and thus, the Proposition applies). Therefore, three voters is insufficient. And we will be able to show that so is four. And five. And six. And seven. And so on, and so on.
...
**Unanimity:** If every voter ranks $a>b$, then $\mu(a>b)=\mu(V)=1$, and hence $\mu(b>a)=\mu(\varnothing)=0$. Thus, $\mu(a>b)=1>0=\mu(b>a)$ so society ranks $a>b$.
