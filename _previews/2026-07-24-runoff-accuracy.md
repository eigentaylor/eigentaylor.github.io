...
toc:
  - name: Introduction
    subsections:
      ...
      - name: The Contenders
      - name: VSE in Context
      - name: Voter Information Quality
  ...
  - name: The Runoff Assumptions
    subsections:
      ...
      - name: Justification
  - name: The Short Version of the Methodology
  - name: Findings
    subsections:
...
Voters in Eugene appeared uninterested in this and overwhelmingly rejected STAR voting in favor of the status quo.

The overwhelming defeat of STAR in Eugene was a shock to many, and a devastating blow to the STAR voting movement. But 2024 was a particularly bad year for expressive voting systems in general. [Ranked-Choice Voting (RCV)](../ditch-rcv/) also faced [a sound rejection at the ballot box in a number of states, including Oregon](https://ballotpedia.org/Results_for_ranked-choice_voting_(RCV)_and_electoral_system_ballot_measures,_2024), and it is entirely possible STAR was caught in the same undertow. But it's also worth asking a separate question: upon what assumptions is the claimed "accuracy" of STAR based?
...
While I was originally strongly against pairing Approval voting with a runoff, due to theoretical loss of certain guarantees criteria<d-cite key="fishburnBrams1981runoff"></d-cite>, the results from this model have completely changed my mind: a delayed runoff appears to significantly improve the accuracy of Approval voting without sacrificing its simplicity. I now believe that Approval Top-2, rather than just being a far more practical alternative which is *almost* as accurate as more complex alternatives (see the image below), might actually be exceptionally *more* accurate at electing better candidates.
...
Already at this point, Approval's Top-2 variant looks accurate enough. But a single-round of Approval falls short; to close the gap from single-round Approval to STAR, a second election *is* required. Therefore, saying STAR is more "accurate" than Approval is not an unfair reading of the data.
...
If you want to skip over the detailed explanation of the model, you can jump directly to a [short version](#the-short-version-of-the-methodology) or just the [simulation results](#findings).
...
If $u_i(c)$ is the true utility of candidate $c$ for voter $i$, then we define a voter's perceived utility as:

$$u'_i(c) = \rho\cdot u_i(c) + \sqrt{1-\rho^2}\cdot \sigma_i \cdot \epsilon_{ic}$$

Simply put, the perceived utility is a noisy version of the true utility<d-footnote>Technically put, $\epsilon_{ic} \sim N(0,1)$ and $\sigma_i$ is the standard deviation of voter $i$'s true utilities across the candidates, used to scale the injected noise to that voter's own utility range. $\rho$ is thus exactly the Pearson correlation coefficient between the true and perceived utilities.</d-footnote>. If $\rho=1$, then the voter is perfectly informed on their exact utilities, and if $\rho=0$, then the voter's perceived utilities are pure noise. We use one global $\rho$ for all voters.
...
<div class="caption mt-2">
  Bobby Newport from Parks and Rec is Leslie Knope's opponent for city council. He has the name recognition and charisma, as the heir to his family's candy business who "employs half the town", which is enough to put him 70 points ahead in the polls the second he announces his candidacy. But he has no qualifications to be a city councilor. Even so, people won't vote for a "lifelong government bureaucrat who's well versed in the issues" like Leslie Knope if they don't know who she is.
</div>
...
At $\alpha=1$, all voters are aware of all candidates, while at lower values, fewer voters are aware of the candidates with a worse prominence ranking.
...
If either check fails, then the utility for that candidate on the input ballot is set to be just below that of their least liked known candidate. This simulates leaving that candidate off of their ballot. For cardinal systems like STAR and Approval, this means giving them a score of 0.
...
Under the most pessimistic conditions, we could imagine that the voter has absolutely no time to update their beliefs. The voter essentially falls into a coma as soon as they submit their ballot, wakes up on election day in November, and then casts their runoff vote. We call this the "coma" runoff assumption, because I think that's kind of funny (and illustrative). In Approval Top-2's coma variant, voters still have the opportunity to vote for their preferred finalist (according to their potentially misinformed preferences) without fatigue, *unless they were unaware of both candidates*.

This does mean that in a coma runoff, a voter will vote for any candidate they are aware of over any candidate they are not. That is, they might vote for the devil they know, even if they would never actually approve of that candidate. The realism of this assumption is certainly debatable, but functions as a maximally pessimistic reference point<d-footnote>It would be interesting to see alternative assumptions, such as only voting for a known candidate if they have above-average utility. That is, thinking "there's no way this other candidate can be as bad as the one I know."</d-footnote>.

An automatic runoff is very different from the coma model, and it was not necessarily clear which would be better. Both systems have the potential for voters to vote against their interests.

- In STAR, a voter might give a corrupt incumbent 1 star, but be too fatigued to give a boring candidate their rightful 3 stars (or be unaware they exist). Then in the automatic runoff, this voter's ballot will be cast for the 1-star candidate over the interpreted 0 stars.
- In coma Approval Top-2, a voter might similarly be unaware of that better candidate and vote for the corrupt incumbent they know (even though they didn't approve them in the primary).
...
The most optimistic assumption is that voters have spent the time between elections to research the candidates, watching the debates, reading the websites, and generally becoming more informed.
...
I will not deny that the clear-eyed assumption is particularly optimistic for a delayed runoff. However, it turns out that the groggy assumption is more than enough to make Approval Top-2 significantly more robust than STAR in this model.
...
## The Short Version of the Methodology

- We simulate misinformation by adding noise to voters' perceptions of candidates' utilities. As $\rho$ decreases, voters become more misinformed.
- We simulate candidates as having different levels of "prominence", leading to some being better known than others. Voters don't vote for candidates they don't know. As $\alpha$ decreases, voters are less likely to know about less prominent candidates.
- Voters get tired as they fill out the ballot and may skip names as they go down the list. As $\beta$ decreases, voters are more likely to skip candidates further down the ballot.
- We simulate multiple runoff models:
  - Coma: Voters don't learn anything between the primary and the runoff.
  - Groggy: Voters are aware of the two finalists but remain misinformed.
  - Clear-eyed: Voters are perfectly informed and vote for the candidate they truly prefer.

Finally, I also defined "joint scenarios" of various friction levels where I set the $\rho=\alpha=\beta$ parameters to the same values:
...
For more information on the methodology, [see the appendix](#other-relevant-methodology).
...
It is absolutely undeniable that, with perfectly informed voters who fill out their ballots completely and accurately, STAR is an objectively more accurate mechanism than Approval in this model. That granularity is a genuine strength when the data is high quality. However, that granularity fails to deliver when the data you collect is noisy or incomplete, making the system no better than the simpler single-round Approval.
...
If there is anything I have taken away from this project, it's that a delayed runoff is of **enormous benefit**. A narrowing process seems absolutely necessary, and using the simple Approval system for that winnowing seems to be the most robust and practical option. The ballot scales exceptionally well to crowded fields, compared to a ranked or scoring ballot, and eliminates the vote splitting we see in the Plurality Top-2 systems used in Washington and California. If 61 candidates on the ballot is a possibility, then Approval is the only alternative system that can handle that without completely overbloating the ballot and overwhelming voters.
...
I cannot speak to if eliminating a primary election could theoretically improve equity. But I also have serious concerns about the potential negative impacts on equity that could result from complicating the ballot and eliminating any sort of winnowing process.

Research on ballot-marking errors<d-cite key="neelyMcDaniel2015overvoting"></d-cite> shows worrying signs. Well-intentioned "expressiveness" has a darker side-effect when looking at RCV in San Francisco: Spoiled ballot rates were disproportionately higher in precincts with with more Black, Latino, elderly, lower-income, and foreign-born residents whose first language is potentially not English.
...
[STAR is a significant improvement in how difficult it is to spoil a ballot over RCV](https://www.starvoting.org/voter_behavior), because voters can give multiple candidates the same score. But it's still entirely possible to accidentally give one candidate multiple scores, which could impact if that ballot is counted correctly or not. It's easy to *say* "just rank your candidates 1, 2, 3..." or "just score your candidates 0 to 5 stars," but when you suggest anything more complex than choose-one or Approval, I think we have to be incredibly mindful of what the cost of complexity could be, and if the theoretical benefits of granularity and complexity justify it. Especially if STAR is implemented without a primary, that means *more* candidates on the ballot, which can make it more intimidating and overwhelming for voters, and easier to make a mistake on that giant grid of bubbles.

I think it is safe to say, however, that Approval is easily the hardest ballot type to spoil, which is a benefit to consider. When a more granular ballot and system fails to justify itself in producing significantly better outcomes, as my analysis suggests, it essentially just creates greater surface area to unintentionally disenfranchise people who don't have the time to treat politics like a hobby.
...
In a 1998 paper by Regenwetter and Grofman, they analyzed the outcomes of real Approval elections to see if they might match the theoretical outcomes under ranked methods like Borda or Condorcet from reconstructed preferences. I find my own conclusion to align with theirs:
...
When we step beyond the idealized assumptions of perfect voters, we find that the "top of the line" systems like STAR perform far worse than the simpler two-round system being used *right now* in St. Louis under even just the groggy assumption. Approval Top-2 may not be a compromise at all, but actually superior and more accurate than the more complex STAR that it has been overlooked in favor of. And when voters have actually been asked to choose STAR for themselves, they've said no every time. Perhaps we've been attempting to overcomplicate our elections, flying too close to the sun chasing perfection. All the while, the most robust solution might just be quietly working in Missouri as we speak.
...
I also discuss any important statistically significant gaps that do not persist under a 99% confidence interval [here](#a-stricter-confidence-interval).

---

Finally, I would like to round off part 1 with a few surprising findings related to choose-one voting.
