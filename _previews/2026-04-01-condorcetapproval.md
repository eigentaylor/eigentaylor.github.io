...
[For an actually serious argument on the fact that Approval is a practical Condorcet approximation, see this post.](../practicalapproval){:target="_blank"}
...
But it is impractical to hold an election for every $\frac{n(n-1)}{2}$ pair of candidates, so we need a voting system that can be used in a single election, but still satisfies the Condorcet criterion: if there is a Condorcet winner, *and voters express this through their ballots*, then they should win.
...
> **Axiom:** If no candidate wins all of their matchups, based on the ballot data, then we make no assumption about which candidate is the Condorcet winner or should win. If $\mathcal{C}$ is the set of all candidates, and $P$ induces no Condorcet winner, then we make no assumption about the outcome of $C_k(P)$.
