...
I am currently writing a post on Arrow's Impossibility Theorem, but I wanted to write a quick post about the killer property that makes that theorem an impossibility: Independence of Irrelevant Alternatives (IIA).
...
If the single voter maintains their preference for B over A, but changes their score for A to be 0, then A would only get 20 points and lose to B, even though the relative ranking of A and B is unchanged for all voters. This violates the strict notion of IIA, in terms of ordinal preferences. Nobody changed their ordinal preference between A and B, but the outcome changed from A winning to B winning.

So long as there is at least one score you can give between the minimum and maximum score, then you can generate an ordinal IIA violation like this. But what if you only have two scores? I encourage you to give it a try and see if you can find a violation!

Notice this would also give that singular voter a beneficial strategy. They clearly prefer B to A, but they needed to exaggerate their score for A to be much lower than they might genuinely feel to get B to win. This is a violation of strategyproofness, which we will talk about in the next section.
...
In a [2007 paper by Vorsatz](https://www.jstor.org/stable/41106808), he proves a connection between strategyproofness and IIA while writing about Approval voting. In any system which is "neutral" (no candidate bias) and strategyproof, we have that the system must satisfy IIA. This is a very strong result, because it means that where there are IIA violations, there are also strategyproofness violations. However, this is a particular statement of a much more general result [by Blair and Muller](https://doi.org/10.1016/0022-0531(83)90092-3) that actually proves a sort of bi-directional connection between strategyproofness and IIA.

The precise definition of strategyproofness is a bit technical, but I explain it in my [post on the Gibbard-Satterthwaite Theorem](../gibbard-satt/){:target="_blank"}. The gist of it is that changing your ballot should never lead to a better result according to your *previous* ballot. For example, if you submit a ballot that says "I like Apple more than Blueberry", then you shouldn't be able to change your ballot and subsequently change the winner from Blueberry to Apple. If you could do that, then you would have an incentive to lie, and a beneficial strategy.
...
Intuitively, this connection actually makes sense. If you could change the outcome between $x$ and $y$ by changing how voters feel about some irrelevant candidate $z$, then that itself creates a strategy where you can get a better outcome depending on how you feel about $x$ and $y$.
...
In their seminal [1978 paper](https://doi.org/10.2307/1955105), Brams and Fishburn show that Approval voting is strategyproof on dichotomous preferences. Since Approval is also neutral, then by Lemma \ref{stratproof-iia} it must satisfy IIA on dichotomous preferences as well! But what does that even mean?
