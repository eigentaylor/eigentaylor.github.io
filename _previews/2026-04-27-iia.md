...
Suppose the 100 guests at the dinner party are split into two groups as follows, denoting $\succ$ as "is strictly preferred to":

| Number of Guests | Preferences                 |
| ---------------- | ----------------------------|
| 61               | Apple $\succ$ Blueberry     |
| 39               | Blueberry $\succ$ Apple     |
...
| Number of Guests | Preferences                                    |
| ---------------- | -----------------------------------------------|
| 30               | **Apple $\succ$ Blueberry** $\succ$ Cherry     |
| 31               | Cherry $\succ$ **Apple $\succ$ Blueberry**     |
| 39               | **Blueberry $\succ$ Apple** $\succ$ Cherry     |
...
There are systems which try to satisfy IIA directly, called "Condorcet" methods, which induce a ranking via direct majority comparisons. For instance, with the above pie scenario, we have that Apple defeats Blueberry and Cherry head to head. So society should rank Apple over Blueberry, and Apple over Cherry. Further, Blueberry defeats Cherry, so society should rank Blueberry over Cherry. This gives a societal ranking of Apple $\succ$ Blueberry $\succ$ Cherry, and Apple wins. However, this is not guaranteed to work!
...
| 30               | Apple $\succ$ Blueberry $\succ$ Cherry     |
| 31               | Cherry $\succ$ Apple $\succ$ Blueberry     |
| 39               | Blueberry $\succ$ **Cherry $\succ$ Apple** |

Now, Apple beats Blueberry 61% to 39%, Blueberry defeats Cherry 69% to 31%, but Cherry defeats Apple 70% to 30%. This is a "Condorcet cycle" (think rock-paper-scissors), which means we can no longer simply induce a ranking via who wins by a majority. Condorcet methods try to remedy this by adding tiebreaking rules. A system like ["Ranked Pairs"](https://electowiki.org/wiki/Ranked_Pairs) locks in the societal ranking via the strongest wins. The two strongest wins are Blueberry $\succ$ Cherry and Cherry $\succ$ Apple, so Ranked Pairs would declare the societal ordering Blueberry $\succ$ Cherry $\succ$ Apple, and Blueberry wins again!
...
| ---------------- | -------------------------------------------| --------------- |
| 30               | Apple $\succ$ Blueberry $\succ$ Cherry     | Apple           |
| 31               | Cherry $\succ$ Apple $\succ$ Blueberry     | Cherry, Apple   |
| 39               | Blueberry $\succ$ Apple $\succ$ Cherry     | Blueberry       |
