...
These, along with Webster, belong to the collection of divisor methods with fixed rounding rules. You pick some divisor $D$, starting with the standard divisor $s=P/M$ where $P$ is the total population and $M$ is the number of seats, and then you divide each state's population $p_j$ by $D$ to get a quotient (ex. $p_j/D=69.4$ means that the state should get seats approximately in that ballpark). Then you round that quotient to get the number of seats for that state. If this doesn't add up to $M$, then you adjust $D$ (increasing if you have too many seats, and decreasing if you have too few) until you get $M$ seats.
...
The mechanism is loosely this: when you increase the number of seats, the remainders get shuffled around. And larger states usually have their fractional parts "leapfrog" the remainders of smaller states. So increasing the House size might bring your remainder from closer to 1 to closer to 0, and you might lose a seat you previously got for having a large remainder. This is a chaotic effect that is hard to predict, and it is a serious problem for the Vinton's method.
...
$$\frac{803,655 - 543,105}{543,105} \approx 0.479$$

while the 8-2 apportionment has a relative gap of about 52.06%:

$$\frac{610,993 - 401,828}{401,828} \approx 0.5206$$

This greater *relative* difference, which the Hill method minimizes, is what makes the method choose the 9-1 apportionment.
