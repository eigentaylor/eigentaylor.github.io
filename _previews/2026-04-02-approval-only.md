...
I am open to considering other systems in the future. If we get significant polarization diffusion and greater trust from voters to use a more complex system like STAR or Condorcet methods, then I am open to revisiting those options as viable alternatives. But in this current moment, I can only say that I support "Approval today, [insert your preferred system for the future here] tomorrow," [so long as that system is not RCV](../ditch-rcv){:target="_blank"}. Approval voting, I think, truly needs to be the bridge. The new baseline from which we can *then* discuss if we want or need something supposedly "better".

## The Fisher Criteria

The Equal Vote Coalition champions three systems: Approval, STAR, and Ranked Robin (AKA Condorcet). I, however, currently only champion one of those: Approval voting. This is not a decision I come to lightly, but rather based on my assessment of the current political and social context in the United States, and the practical realities of what implementation involves. Based on the criteria that I feel are most important, only one system uniquely satisfies all of them.
...
My argument is simple: let us begin with the sturdy, stable, boring bridge that Approval voting *can provide today* away from choose-one plurality towards a better place where we can start having a serious conversation about whether we want to move to something more complex like STAR or Condorcet methods in the future.
